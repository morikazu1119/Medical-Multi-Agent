...
MODEL_DIR = "/mnt/slum/models/medgemma-4b-it"  # ★モデル配置先
QUANT_MODE = "4bit"  # "4bit" / "8bit" / "bf16"
MAX_TOKENS = 512  # 生成トークン長
MCP_NAME = "medgemma-4b-it-mcp"
...
def diagnose(request: MCPRequest) -> MCPResponse:
    ...
    proc = mp.Process(target=_infer_worker, args=(child_conn, messages, QUANT_MODE))
    proc.start()
...
# -------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="MedGEMMA-4B-IT-BNB FastMCP Server")
    parser.add_argument(
